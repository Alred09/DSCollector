...
def export_json(chat_id):
    db = create_session()
    selected_chats = request.args.getlist('selected_chats')  # получаем список из GET-параметров
    ...
    try:
        ...
        # Для экспорта выбранных чатов (через GET-параметры)
        elif selected_chats:
            all_messages = []
            for chat_id_single in selected_chats:
                chat = db.query(TelegramChat).filter_by(chat_id=chat_id_single).first()
                if not chat:
                    db.close()
                    return f"Чат {chat_id_single} не найден", 404
            ...
            filename = f"export_multiple_chats_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.json"
